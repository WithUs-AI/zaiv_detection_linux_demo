...
public:
    HailoTileROI(HailoBBox bbox, uint32_t index, float overlap_x_axis, float overlap_y_axis, uint32_t layer, hailo_tiling_mode_t mode) : HailoROI(bbox), m_index(index), m_overlap_x_axis(overlap_x_axis), m_overlap_y_axis(overlap_y_axis), m_layer(layer), m_mode(mode){};
    // Move constructor
    ...
    uint32_t get_layer() { return m_layer; }
