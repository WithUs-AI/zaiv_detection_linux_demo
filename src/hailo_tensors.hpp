...
public:
     ...
     */
    uint8_t get(uint32_t row, uint32_t col, uint32_t channel)
    {
        uint32_t width = m_vstream_info.shape.width;
        uint32_t features = m_vstream_info.shape.features;
        int pos = (width * features) * row + features * col + channel;
        return m_data[pos];
    }
    uint16_t get_uint16(uint32_t row, uint32_t col, uint32_t channel)
    {
        uint32_t width = m_vstream_info.shape.width;
        uint32_t features = m_vstream_info.shape.features;
        int pos = (width * features) * row + features * col + channel;
     ...
     */
    float get_full_percision(uint32_t row, uint32_t col, uint32_t channel, bool is_uint16)
    {
        if (is_uint16)
